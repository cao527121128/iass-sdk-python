...
def modify_snapshot_attribute(snapshots_id):
    print("子线程启动")
    print("modify_snapshot_attribute")
    ...
    #修改指定备份的相关属性
    ret = conn.modify_snapshot_attributes(snapshot=snapshots_id_string,snapshot_name="snapshot-vdi-1731",description="snapshot description")
    ...
    print("ret_code==%s" % (ret_code))
    if ret_code != 0:
        print("modify_snapshot_attributes failed")
        exit(-1)
...
if __name__ == "__main__":
    ...
    #创建子线程--修改指定备份的相关属性
    t = threading.Thread(target=modify_snapshot_attribute,args=(snapshots_id,))
    t.start()
    t.join()
