...
class APIConnection(HttpConnection):
    ...
    def describe_cache_nodes(self, caches=None,
                        status=None,
                        verbose=0,
                        owner=None,
                        search_word=None,
    ...
    def describe_dev_apps(self,
        ...
        valid_keys = ['app', 'app_type', 'status', 'sort_key', 'owner', 'verbose', 'search_word', 'offset', 'limit',
                      'reverse']
        body = filter_out_none(locals(), valid_keys)
        if not self.req_checker.check_params(body,
                                             required_params=[],
    ...
    def describe_cluster_nodes(self,
                               ...
                               reverse=None,
                               **ignore):
        """ describe cluster_nodes filtered by condition.
        @param cluster:The ID of cluster.
        @param cluster_nodes:Cluster node ID.
        @param role: Cluster node role.
        @param status: ["active","stopped","pending"].
        @param sort_key: the sort key, which defaults be create_time.
        @param owner: Filter by user account, return only the resources of the specified account.
        @param verbose: the number to specify the verbose level,
        ...
        valid_keys = ['cluster', 'cluster_nodes', 'role', 'status', 'sort_key', 'owner', 'verbose', 'search_word',
