...
def create_rdb(conn,user_id,vxnet_id,master_private_ip,topslave_private_ip):
    ...
    if status == "successful":
        ...
        print("action == %s" % (action))
        ret = conn.describe_tags(search_word='桌面云数据库',offset=0,limit=100)
        ...
        if tag_set is None or len(tag_set) == 0:
            ...
            print("action == %s" % (action))
            ret = conn.create_tag(tag_name='桌面云数据库')
        ...
        print("action == %s" % (action))
        resource_tag_pairs = [{"resource_type": "rdb", "resource_id": rdb_id, "tag_id": tag_id}]
        selectedData = [tag_id]
