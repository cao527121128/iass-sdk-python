...
def create_volume_from_snapshot(snapshots_id):
    print("子线程启动")
    ...
    #将指定备份导出为硬盘
    ret = conn.create_volume_from_snapshot(snapshot=snapshots_id_string,volume_name="volume_from_snapshot_1748")
    ...
    print("ret_code==%s" % (ret_code))
    if ret_code != 0:
        print("create_volume_from_snapshot failed")
        exit(-1)
    ...
    if ret.get("ret_code") == 0:
        ...
        while num < 300:
            ...
            if status == "successful":
                print("create_volume_from_snapshot successful")
                break
    #check volume_id
    volume_id = ret.get("volume_id",0)
...
if __name__ == "__main__":
    ...
    #创建子线程--将指定备份导出为硬盘。请注意，此备份点必须为硬盘的备份点才能导出为硬盘，而且通过备份创建的硬盘类型和备份的来源硬盘类型是一致的。
    t = threading.Thread(target=create_volume_from_snapshot,args=(snapshots_id,))
    t.start()
    t.join()
