...
def apply_snapshot(snapshots_id):
    print("子线程启动")
    print("apply_snapshot")
    print("snapshots_id == %s" %(snapshots_id))
    user_id = get_user_id()

    #删除备份
    ret = conn.apply_snapshots(snapshots=snapshots_id)
    ...
    if ret.get("ret_code") == 0:
        ...
        while num < 300:
            ...
            if status == "successful":
                print("apply_snapshots successful")
                break
...
if __name__ == "__main__":
    ...
    #创建子线程--回滚到指定备份点
    t = threading.Thread(target=apply_snapshot,args=(snapshots_id,))
    t.start()
    t.join()
